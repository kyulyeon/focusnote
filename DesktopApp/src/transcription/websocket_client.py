        ...
        while self.running:
            try:
                async with websockets.connect(self.server_url) as websocket:
                    ...
                    audio_buffer = []
                    target_samples = 16000 * 2  # 2 seconds at 16kHz (smaller chunks)
                    ...
                    while self.running:
                        ...
                        total_samples = total_bytes // (channels * 2)  # 2 bytes per sample (int16)
    ...
    async def _send_buffer(self, audio_buffer, sample_rate, channels):
        ...
        try:
            ...
            if len(float32_data) < min_samples:
                print(f"⚠️  Buffer too short ({len(float32_data)} samples), padding to {min_samples}")
            ...
            max_samples = int(16000 * 5)
            if len(float32_data) > max_samples:
                print(f"⚠️  Buffer too long ({len(float32_data)} samples), truncating to {max_samples}")
