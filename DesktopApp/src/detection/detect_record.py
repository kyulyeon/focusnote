...
class AudioCapture:
    ...
    def get_process_names(self, base_names):
        ...
        if SYSTEM == "Windows":
            return [f"{name}.exe" if not name.endswith('.exe') else name for name in base_names]
        return base_names
    ...
    def start_recording(self, platform_name=None):
        ...
        def record():
            ...
            try:
                ...
                # Open speaker stream
                if self.speaker_device:
                    channels_spk = min(self.speaker_device.get('maxInputChannels', 2), 2)
                    rate_spk = int(self.speaker_device.get('defaultSampleRate', self.rate))
                ...
                while self.is_recording and recording_active:
                    try:
                        ...
                        if self.stream_speaker:
                            try:
                                speaker_data = self.stream_speaker.read(self.chunk, exception_on_overflow=False)
                            except:
                                pass
                        ...
                        if self.stream_mic:
                            try:
                                mic_data = self.stream_mic.read(self.chunk, exception_on_overflow=False)
                            except:
                                pass
        ...
        while self.running:
            ...
            if not self.in_call:
                ...
                if is_any_call_active:
                    ...
                    print(f"🔍 Call activity ({self.call_detected_count}/{self.call_detection_threshold})")
                    sys.stdout.flush()
                ...
                if is_original_call_active:
                    ...
                    if self.inactive_count % 3 == 0:
                        print(f"⏳ Inactive {self.inactive_count}/{self.inactive_threshold}s")
                        sys.stdout.flush()
